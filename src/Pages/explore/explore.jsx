...
const data = [
  {
    name: "Steel",
    description: "An alloy of iron and carbon, widely used in construction and manufacturing due to its strength and durability.",
...
function Explore() {
  return (
    <>
      ...
      <Navbar></Navbar>
      <div>
        <h1 className="text-white text-center text-4xl">This is Explore page</h1>
        <div className="material-list flex flex-col lg:flex-nowrap items-center justify-center my-5 gap-5">
          {data.map((item, index) => (
            <ExploreCard key={index} name={item.name} description={item.description} link={item.link} photo className="material-item"/>
          ))}
        </div>
      </div>
      <Footer></Footer>
